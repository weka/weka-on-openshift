...
##Best used on a free AWS Graviton bastion host###
...
echo "Dowloading and extracting Openshift installer version ${OCP_VERSION} for Linux aarch64..."
command wget -nc https://mirror.openshift.com/pub/openshift-v4/aarch64/clients/ocp/${OCP_VERSION}/openshift-install-linux.tar.gz
command tar xzvf openshift-install-linux.tar.gz openshift-install
command chmod u+x openshift-install
command cp openshift-install ~/bin/

echo "Downloading oc client version ${OCP_VERSION} for Linux aarch64..."
command wget -nc https://mirror.openshift.com/pub/openshift-v4/aarch64/clients/ocp/${OCP_VERSION}/openshift-client-linux.tar.gz
command tar xzvf openshift-client-linux.tar.gz oc
